...
then that is used instead.''',
      ...
      ..addOption(
        'depfile',
        valueHelp: 'path',
        help: 'Path to an output file in Ninja depfile format containing a list of compilation dependencies. This is passed to the compiler to support dependency tracking in build systems.',
      )
      ..addOption(
        'target-sanitizer',
  ...
  Future<int> run() async {
    ...
    if (target == null) {
      if (entryPoints.isEmpty) {
        stderr.writeln("No entry point was specified. Use '--target <path>'.");
      } else {
        stderr.writeln(
    ...
    final sourceUri = File.fromUri(Uri.file(target).normalizePath())
        .absolute
        .uri;
...
Use linkMode as dynamic library instead.""",
        ...
        await snapshotGenerator.generate(
          ...
          extraOptions: [...sanitizer.genSnapshotFlags],
        );

        if (busyBoxStyle) {
          if (first) {
            await File(sdk.dartCliRuntimeFor(sanitizer: sanitizer.name))
                .copy(outputExeUri.toFilePath());
            await markExecutable(outputExeUri.toFilePath());
          } else {
            await Link.fromUri(outputExeUri)
