...
if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
  echo "SKIP: upstream changes audit skipped in GitHub Actions CI"
else
...
starlark_cp_matches=$(git grep -n -E 'ctx\.execute\(.*"cp"' -- '*.bzl' 2>/dev/null | grep -v '^third_party/' | grep -v '# exempt-starlark-copy: ok' || true)
