...
# 5. Delegate to test_runner.dart & render markdown matrix
RC=0
"$DART" tools/bazel/testing/test_runner.dart "${PASSTHROUGH_ARGS[@]}" || RC=$?
if [ -f "$OUTPUT_PATH" ]; then
  "$DART" tools/bazel/testing/render_matrix.dart "$OUTPUT_PATH" docs/bazel-migration/TEST_COMPLETION_MATRIX.md
fi
exit "$RC"
